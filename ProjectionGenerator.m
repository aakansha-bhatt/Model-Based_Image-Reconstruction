...
for i = 1:N
    pp = radon(scan(:,:,i));
    nofilt = iradon(pp,[0:179]);
    nofilt = nofilt(2:129, 2:129); %recenters it
    fbp_ram = iradon(pp,[0:179],'linear','ram-lak',1,dim);
    fbp_shepp = iradon(pp,[0:179],'linear','shepp-logan',1,dim);
    fbp_cos = iradon(pp,[0:179],'linear','cosine',1,dim);
    fbp_hamm = iradon(pp,[0:179],'linear','Hamming',1,dim);
...
save('N_all_inputs.mat', 'all_img_inputs');
save('N_all_outputs.mat', 'all_img_outputs');
